...
impl ModuleTreeConfig {
    ...
    pub fn new(module_string : &str, write_main : bool) -> ModuleTreeConfig {
        ...
        if modules_and_lists.len() == 2 {
            ...
            let sub_modules_names : BTreeSet<String> = modules_and_lists[1].to_string().split(',').map(|md| md.trim().to_owned()).collect::<BTreeSet<String>>();
            ModuleTreeConfig {
